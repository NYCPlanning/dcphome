function initShareLinks(){
	/*$(".twitter_custom").each(function(){
		...
		$.getJSON("http://api.bitly.com/v3/shorten?callback=?", 
		    { 
		        ...
		        "login": "nycgov",
		        "longUrl": convertIP($(this).data().url)
		    },
		    function(response){
	...
	$(".twitter_custom").each(function(){
		...
		$(this).parent("a").attr("href", "http://twitter.com/home?status="+twitterDescription+" "+convertIP($(this).data().url)+" via @nycgov");
	});
...
function convertIP(theUrl){
	return(theUrl.replace('http://157.188.76.61:8004','http://www1.nyc.gov').replace('http://10.155.20.219:8004','http://www1.nyc.gov').replace('http://157.188.76.62:8004','http://www1.nyc.gov'));
}
